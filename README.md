...
- 좌측 사이드바 폭과 워크스페이스 버튼 내부 여백 재조정
- 현재 워크스페이스 이름과 경로를 함께 표시
- 선택 팝업을 최대 420px 반응형으로 확대하고 긴 경로 줄바꿈
- 폴더 목록만 스크롤하고 추가·탐색기 버튼은 항상 보이도록 고정
- 팝업의 좌우·상하 화면 경계 및 창 크기 변경 위치 보정
- v0.1.22의 앱 내부 자동 업데이트와 설치기 직접 다운로드 경로 유지
