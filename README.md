...
- 단일·분할 화면 모두 휠이 실제 대상 터미널을 스크롤하도록 입력 경로 통일
- 분할 화면에서 바깥 그리드가 휠 입력을 가로채는 문제 수정
- 휠을 올린 분할 터미널을 자동 활성화하고 TUI mouse-reporting 유지
- 터미널 셀의 최소 높이·overflow를 조정해 분할 시 xterm viewport 정상 배치
- 긴 AI 작업의 상단 내용 보존을 위해 스크롤백을 5,000줄에서 20,000줄로 확대
- 기존 앱 내부 업데이트와 수동 설치기 다운로드 경로 유지
